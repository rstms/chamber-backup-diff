...
import chamber_backup_diff
from chamber_backup_diff import __version__, cli


def test_version():
    """Test reading version and module name"""
    assert chamber_backup_diff.__name__ == "chamber_backup_diff"
    assert __version__
...
def test_cli_diff(run, shared_datadir):
    old = shared_datadir / "old.tgz"
    new = shared_datadir / "new.tgz"

    result = run([str(old), str(new)])
    assert result
